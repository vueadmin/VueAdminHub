<template>
  <div class="px-4 sm:px-0">
    <!-- Page Header -->
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-vue-dark mb-4">🎨 UI 组件库</h1>
      <p class="text-gray-600">
        精选的 Vue.js UI 组件库，提供丰富的组件和优秀的用户体验
      </p>
    </div>
    ...
    <SearchBanner
      title="寻找完美的 UI 组件库"
      placeholder="搜索 UI 组件库..."
      :popular-tags="['Element Plus', 'Ant Design Vue', 'Naive UI', 'Vuetify', 'Quasar']"
      @search="handleSearch"
    />

    <ResourceFilter
      :categories="uiLibraryCategories"
      @filter="handleFilter"
    />
    ...
    <div class="mb-6 flex justify-between items-center">
      <div class="text-sm text-gray-500">
        找到 {{ filteredResources.length }} 个组件库
      </div>
    ...
    <div v-if="filteredResources.length === 0" class="text-center py-12">
      ...
      <h3 class="text-lg font-medium text-gray-900 mb-2">未找到匹配的组件库</h3>
...
import { uiLibraries } from '~/data/resources'
...
// UI Library-specific categories
const uiLibraryCategories = computed(() => {
  const uiLibraryCount = uiLibraries.length
  return [
    { id: 'all', name: '全部', count: uiLibraryCount },
    { id: 'enterprise', name: '企业级', count: uiLibraryCount },
  ]
})
